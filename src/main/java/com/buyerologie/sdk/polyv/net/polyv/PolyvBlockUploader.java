...
public class PolyvBlockUploader {
     ...
     *            上传监听器
     */
    public PolyvBlockUploader(String luping, String filepath, String cataid, String title,
                              String tag, String vid, String userid, String writetoken,
                              JSONObjectRet jsonObjectRet) {
    ...
    public PutExtra getPutExtra(String bucketName, String fileKey) {
        ...
        File configFile = new File(System.getProperty("user.dir") + File.separator + infofileParent
    ...
    private boolean readData(HttpURLConnection conn, String url) {
        ...
        try {
            ...
            System.out.println(filename + "获取uploadtoken时解析json发生异常：" + e + " json数据为："
                               + sb.toString());
    ...
    private boolean getUploadToken(String url) {
        try {
            ...
            if (reGetUploadToken > 0)
                System.out.println(filename + "第" + reGetUploadToken + "次请求获取uploadtoken时发生了异常："
                                   + e);
            if (reGetUploadToken < 0) {
    ...
    public boolean upload() {
        ...
        switch (status) {
            ...
            case GET_UPLOADTOKEN_SUCCESS:
                sliceUploadResumable.execUpload(bucketName, fileKey, filepath, putExtra,
                    jsonObjectRet, uploadToken);
                return true;
        }
        return true;
    }
